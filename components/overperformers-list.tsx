...
interface OverperformersListProps {
    allPlayers: Player[];
    teams: Team[];
    fixtures: Fixture[];
...
export function OverperformersList({ allPlayers, teams, fixtures, onPlayerClick, playerHistories }: OverperformersListProps) {
    const [showBadFixturesOnly, setShowBadFixturesOnly] = useState(false);
        ...
        <Card>
            <CardHeader>
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <CardTitle>📈 Overperformers Watchlist (Potential Traps)</CardTitle>
                        <CardDescription>
                            Players with low Expected Goal Involvement (xGI) but high actual returns.
                            Statistically likely to decline ("regress to the mean").
                        </CardDescription>
                    </div>
                    <div className="flex items-center space-x-2 bg-muted/50 p-2 rounded-lg">
                        <Switch
                            id="fixtures-filter"
                            checked={showBadFixturesOnly}
                            onCheckedChange={setShowBadFixturesOnly}
                        />
                        ...
                        {overperformers.length === 0 ? (
                            <TableRow>
                                <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                                    No significant overperformers found matching criteria.
                                </TableCell>
                            </TableRow>
                        ) : (
                            overperformers.map(({ player, xGI, actualGI, delta, upcoming }) => (
                                <TableRow key={player.id}>
                                    ...
                                    <TableCell className="text-right">
                                        <Badge variant="secondary" className="bg-red-100 text-red-800 hover:bg-red-200 dark:bg-red-900/30 dark:text-red-300">
                                            {delta.toFixed(2)}
                                        </Badge>
                                    </TableCell>
                                    <TableCell>
